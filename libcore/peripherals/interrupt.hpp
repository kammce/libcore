...
class InterruptController : public Module<>
{
 ...
 public:
  ...
  struct RegistrationInfo_t
  {
    ...
    /// Pure void(*)(void) function pointer used for setting entries in the IVT
    /// directly vs via a handler. This is requied for vector tables where
    /// having a handler before the vector is called will cause issues, such as
    /// an fault handler or some handlers used by an scheduler/RTOS.
    InterruptVectorAddress interrupt_vector = nullptr;
  };
